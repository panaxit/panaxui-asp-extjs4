...
<xsl:template mode="itemWidth" match="Evento/px:fields/Configuracion">600</xsl:template>
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				...
				newRecord.data["VoluntariosSinGrupo.Voluntario"]=rec.get("VoluntariosEnGrupo.Voluntario")
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["VoluntariosEnGrupo.Voluntario"]=rec.get("VoluntariosSinGrupo.Voluntario")
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["NoParticipantesEnGrupos.PcD"]=rec.get("ParticipantesEnGrupo.PcD")
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["ParticipantesEnGrupo.PcD"]=rec.get("NoParticipantesEnGrupos.PcD")
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["GruposSinAsignar.<xsl:value-of select="$field"/>"]=(rec.store.model.modelName=='EventosPcD.AsignacionEnPrivado'?rec.get("AsignacionEnPrivado.<xsl:value-of select="$field"/>"):rec.get("AsignacionEnPublico.<xsl:value-of select="$field"/>"))
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["AsignacionEnPublico.<xsl:value-of select="$field"/>"]=(rec.store.model.modelName=='EventosPcD.AsignacionEnPrivado'?rec.get("AsignacionEnPrivado.<xsl:value-of select="$field"/>"):rec.get("GruposSinAsignar.<xsl:value-of select="$field"/>"))
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
...
,viewConfig: {
	...
	listeners: {
		...
		beforedrop: function(targetNode, dragData, overModel, position, dropHandler, eOpts) {
			...
			Ext.Array.each(dragData.records, function(rec, index, records) {
				var newRecord=Ext.create(grid.getStore().model.modelName, {})
				newRecord.data["AsignacionEnPrivado.<xsl:value-of select="$field"/>"]=(rec.store.model.modelName=='EventosPcD.AsignacionEnPublico'?rec.get("AsignacionEnPublico.<xsl:value-of select="$field"/>"):rec.get("GruposSinAsignar.<xsl:value-of select="$field"/>"))
				newRecord.data["checked"]=true
				records[index]=newRecord
				rec.store.remove(rec);
			});
