...
try
	{
	dataTable.style.cellPadding=0
	dataTable.style.cellSpacing=0
...
window.attachEvent("onafterprint", freezeHeader);
//attachEvent("onscroll", headerFlotante);
attachEvent("onscroll", scrollUnaVez); 

try
	{
/* boton submit--> */
	formulario.attachEvent("onsubmit", checar_errores);

	var vinculos = document.body.all.tags("A");
	if (vinculos.length)
		{
		for (v=vinculos.length-1; v>=0; --v)
			{
			if (isNaN(parseInt(vinculos(v).innerText))==false)
				{
				vinculos(v).href+="&Registros="+String(vinculos(v).innerText)
				}
			}
		}
	}
catch(e) {}
//	document.body.attachEvent("oncontextmenu", DerechosReservados);
try
	{
	attachEvent("onscroll", botonFlotante);
	botonFlotante()
	
	attachEvent("onscroll", tablaOFlotante);
	tablaOFlotante()
	
	attachEvent("onscroll", tablaOFlotanteFiltros);
	tablaOFlotanteFiltros()
	}
catch(e) {}

try {
	with (top.frames('logo'))
		{
		printer.style.visibility='visible';
		lupa.style.visibility='visible';
		aExcel.style.visibility='visible';
		actualizar.style.visibility='visible';
		BarraProg.style.display='inline';
		BarraProg.style.visibility='hidden';
		}
	}
catch (e) {}
}
...
function freezePaneles()
{
try 
	{
	status="Congelando paneles..."
	num_reg=(freeze?(freeze.length?freeze.length:1):0);
	partes=10;
	num=Math.ceil(num_reg/partes);
	for (f=0; f<num_reg; ++f)
		{
		if (f%num==0 || f==num_reg) status='Congelando paneles... '+Math.round((f+1)*100/num_reg)+'%';
		freeze_obj=freeze.length?freeze(f):freeze;
/*		if (freeze_obj.tagName=='TH' || freeze_obj.tagName=='TD')
			{
			freeze_obj.style.borderColor='silver';
			freeze_obj.style.borderStyle='solid';
//			freeze_obj.style.borderWidth='thin';
			freeze_obj.style.backgroundColor='navy';
			freeze_obj.style.color='white';
			}
		freeze_obj.style.position='relative';*/
		}
	status="Congelando paneles... Completado"
	}
catch(e) { status='Congelando paneles... Falló'; /*alert(e.description); */}
}
...
//funcion para obligar que el evento onscroll solo se ejecute una vez y no 3
var intervalo="";
function scrollUnaVez()
{
try
	{
	if (document.all('header') && dataTable)
		{
		if (intervalo=="")
			{
			intervalo=window.setTimeout("headerFlotante(); window.clearTimeout(intervalo); intervalo=''", 0);
			}
		}
	}
catch (e) {}		
...
function freezeFlotante(obj)
{
for (f=0; f<(obj.all('freeze')?(obj.all('freeze').length?obj.all('freeze').length:1):0); ++f)		
	{
	freeze_cll=obj.all('freeze');
	freeze_obj=freeze_cll.length?freeze_cll.item(f):freeze_cll;
	freeze_obj.style.pixelLeft=document.body.scrollLeft;
	}
}

function headerFlotante(alinear)
{
...
try
	{
	altura_header=0;
	for (h=0; h<(header?(header.length?header.length:1):0); ++h)
		{
//		status="header: "+h
		header_row=header.length?header(h):header;
		header_row.style.pixelTop=document.body.scrollTop;
		altura_header+=header_row.clientHeight
		freezeFlotante(header_row)
		}
	}
catch(e) {}
...
try
	...
	if (dataTable.clientWidth>document.body.clientWidth || alinear || dataTable.all.tags('SELECT'))
		{
		while ( primer_row>0 && (dataTable.rows(primer_row).id=='header' || parseFloat(dataTable.rows(primer_row).offsetTop)>parseFloat(document.body.scrollTop)-15 ) )
			{
//			status='primer_row: '+primer_row
			--primer_row
			}

		while ( primer_row<dataTable.rows.length && (dataTable.rows(primer_row).id=='header' || parseFloat(dataTable.rows(primer_row).offsetTop)<parseFloat(document.body.scrollTop)-15 ) )
			{
//			status='primer_row: '+primer_row
			++primer_row
			}
...
//			status='ultima_row: '+ultima_row
			if (cancelar_headerFlotante)
				{
				cancelar_headerFlotante=false
				return;
				}
			freezeFlotante(dataTable.rows(ultima_row))
...
window.setTimeout(function() {goToFirstVisibleObject(document.body);}, 100);
}
...
try 
	{
	$(":input:visible:first", oTarget).focus()
	}
catch(e) {}
...
else if (!(event.ctrlKey) && event.shiftKey && event.altKey && sKeyCode == 39) //ctrl + flecha abajo
	{
	scrollBy(20, 0)
	event.keyCode=0;
	event.returnValue=false;
	}
else if (!(event.ctrlKey) && event.shiftKey && event.altKey && sKeyCode == 40) //ctrl + flecha izquierda
	{
	scrollBy(0, 20)
	event.keyCode=0;
	event.returnValue=false;
...
{
var oImg=document.createElement('<img id="img_buscando" src="../../../../Images/Advise/in_progress.gif" alt="TRABAJANDO..." width="16" height="16" border="0" style="position:\'absolute\'" onMouseOver="/*this.removeNode(true);*/">')
oSource.insertAdjacentElement("afterEnd", oImg);
}
function ClearInProgress(oSource)
{
var oImg=(oSource.nextSibling && oSource.nextSibling.id=='img_buscando')?oSource.nextSibling:undefined;
if (oImg) oImg.removeNode(true);
}


function DBScalarFunction(oSource)
{
var sSQLQuery = arguments[1]
for (a=2; a<arguments.length; ++a)
	{
	sSQLQuery+= ((a>2?', ':'')+( isObject(arguments[a])?getVal(arguments[a]):(esVacio(arguments[a])?'NULL':(IsNumericOrMoney(arguments[a])?arguments[a]:"'"+arguments[a]+"'"))) )
	}
sSQLQuery+=')'
var oTemp = new Object();
ajaxRequest("../Scripts/ajax_DBScalarFunction.asp?strSQL="+sSQLQuery, oTemp, undefined, false, 'POST')
return oTemp.value
}

function DBProcedure(oSource)
{
...
ajaxRequest/*alert*/("../Scripts/ajax_DBProcedure.asp?strSQL="+sSQLQuery, oTemp, undefined, false, 'POST')
return oTemp.value
}
...
if ((typeof objeto)=='object')
	...
	else if (objeto.length && objeto(0).tagName=='INPUT' && (objeto(0).type=="radio" || objeto(0).type=="checkbox" ))
		{
	...
	else if (objeto.tagName=='INPUT' && objeto.type=="checkbox" && objeto.id!='identifier')
		{
...
{
digitos_decimales==undefined?digitos_decimales=2:null;
...
digitos_decimales==undefined?(digitos_decimales= String(cantidad).length-pos_char-1):null;
multiplicador=Math.pow(10, digitos_decimales)
decimales=String( (Math.round(cantidad*multiplicador))%multiplicador )
/*if (cantidad>=1669982 && cantidad<1669983)
	alert('debugging '+cantidad+', '+((cantidad*multiplicador)%multiplicador)+': '+decimales)*/
...
{
status='Actualizando indices de celdas... '
try
	{
	fila=baseRow;
	tabla=getParent('TABLE', fila);
	cll_Ids=tabla.all('row_Id')
	if (cll_Ids)
		{
		indiceObj=getIndex(fila.all('row_Id'), cll_Ids); 
	
		for (i=indiceObj; i<cll_Ids.length; ++i)
			{
			if (esVacio(cll_Ids(i).innerText)) continue;
			next_value=nextValue(cll_Ids(i), -1)
			cll_Ids(i).innerText=next_value?(next_value+1):1;
	/*		tabla.rows(r+1).all('row_Id')?tabla.rows(r+1).all('row_Id').innerText=( tabla.rows(r).all('row_Id')?parseInt(tabla.rows(r).all('row_Id').innerText)+1:parseInt(tabla.rows(r+1).all('row_Id').innerText)-1) : r=tabla.rows.length-1;*/
			}
		status='Actualizando indices de celdas... Completado'
		}
	}
catch(e) { /*alert(e.description);*/ status='Actualizando indices de celdas... Falló' }
}
...
attachEvent("onbeforeunload", avisaCambios)
function avisaCambios()
{
if (!saltar_cambios && document.submitButton && (/*document.all('IdDetalleEtapa') || */document.all('identifier')) /*&& document.formulario*/ /*&& document.formulario.target=='transacciones'*/ && document.submitButton.uncommitedChanges>0)
	{
	event.returnValue = ('Todavía quedan '+document.submitButton.uncommitedChanges+' cambios sin guardar!, desea DESCARTAR los cambios?');
	}
/*else
	{
...
{
sReturnValue=String(( (eval( ( elemento.getAttribute("db_column_value") || '').replace(/this/gm, "elemento"))) || 
...
switch (data_type)
	...
	case 'moneda':
		...
		val=format_currency(unformat_currency(val));
		break;
	case 'numero':
...
{
var html="";
html+='<TABLE STYLE="border:1pt solid #808080; color:white" BGCOLOR="navy" WIDTH="140" HEIGHT="120" CELLPADDING="0" CELLSPACING="1" onContextMenu="javascript: return false">';
...
html+='/'+' -'+'->\n';
html+='</'+'SCRIPT>\n';
//html+='<TR><TD STYLE="border:1pt solid navy" ID="i1" ONMOUSEOVER="document.all.i1.style.background=\'#CFD6E8\';document.all.i1.style.border=\'1pt solid #737B92\';" ONMOUSEOUT="document.all.i1.style.background=\'navy\';document.all.i1.style.border=\'1pt solid navy\';" ONCLICK="alert(this.parentNode.parentNode.parentNode.parentNode.parentNode.innerHTML)/*window.history.go(1);*/">&nbsp;<IMG SRC="menuforward.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label>Forward</label></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy"><IMG SRC="pixel.gif" WIDTH="130" HEIGHT="1"></TD></TR>';

/*html+='<TR><TD STYLE="border:1pt solid navy" ID="i4" ONMOUSEOVER="document.all.i4.style.background=\'#CFD6E8\';document.all.i4.style.border=\'1pt solid #737B92\';" ONMOUSEOUT="document.all.i4.style.background=\'navy\';document.all.i4.style.border=\'1pt solid navy\';" ONCLICK="window.parent.external.AddFavorite(window.top.location.href,window.top.document.title);">&nbsp;<IMG SRC="menufavorite.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;Add to Favorites...</TD></TR>';
*/
/*html+='<TR><TD STYLE="border:1pt solid navy" ID="i5" ONMOUSEOVER="document.all.i5.style.background=\'#CFD6E8\';document.all.i5.style.border=\'1pt solid #737B92\';" ONMOUSEOUT="document.all.i5.style.background=\'navy\';document.all.i5.style.border=\'1pt solid navy\';" ONCLICK="window.parent.location=\'view-source:\'+window.parent.location.href;">&nbsp;<IMG SRC="menusource.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;View Source</TD></TR>';

html+='<TR><TD STYLE="border:1pt solid navy"><IMG SRC="pixel.gif" WIDTH="130" HEIGHT="1"></TD></TR>';
*/
/*------------------------------------------------------------------------------------------------------------------------------*/
/*html+='<TR><TD STYLE="border:1pt solid navy" ID="i11" ONMOUSEOVER="document.all.i11.style.background=\'#CFD6E8\'; document.all.i11.style.border=\'1pt solid #737B92\';document.all.i11.style.color=\'black\';" ONMOUSEOUT="document.all.i11.style.background=\'navy\';this.style.border=\'1pt solid navy\';document.all.i11.style.color=\'white\';document.all.i11.style.fontSize=\'10pt\'" ONCLICK="if (window.parent.formulario.action!=\'\'){window.parent.formulario.submit(); window.parent.oPopup.hide()} else {alert(\'Esta pagina no acepta guardar cambios\')}" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/guardar.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Guardar</label></TD></TR>';
*/
html+='<TR><TD STYLE="border:1pt solid navy" ID="btnBack" ONMOUSEOVER="document.all.btnBack.style.background=\'#CFD6E8\'; document.all.btnBack.style.border=\'1pt solid #737B92\';document.all.btnBack.style.color=\'black\';" ONMOUSEOUT="document.all.btnBack.style.background=\'navy\';this.style.border=\'1pt solid navy\';document.all.btnBack.style.color=\'white\';document.all.btnBack.style.fontSize=\'10pt\'" ONCLICK="window.parent.history.go(-1);" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/Buttons/btn_LArrow.png" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Regresar</label></TD></TR>';

html+='<TR><TD STYLE="border:1pt solid navy" ID="btnRefresh" ONMOUSEOVER="document.all.btnRefresh.style.background=\'#CFD6E8\';document.all.btnRefresh.style.border=\'1pt solid #737B92\';document.all.btnRefresh.style.color=\'black\';" ONMOUSEOUT="document.all.btnRefresh.style.background=\'navy\';document.all.btnRefresh.style.border=\'1pt solid navy\';document.all.btnRefresh.style.color=\'white\';document.all.btnRefresh.style.fontSize=\'10pt\'" ONCLICK="window.parent.document.location.href=window.parent.document.location.href; window.parent.oPopup.hide()" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/Buttons/btn_refresh.png" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Actualizar</label></TD></TR>';

html+='<TR><TD STYLE="border:1pt solid navy" ID="btnExcel" ONMOUSEOVER="document.all.btnExcel.style.background=\'#CFD6E8\'; document.all.btnExcel.style.border=\'1pt solid #737B92\';document.all.btnExcel.style.color=\'black\';" ONMOUSEOUT="document.all.btnExcel.style.background=\'navy\';this.style.border=\'1pt solid navy\';document.all.btnExcel.style.color=\'white\';document.all.btnExcel.style.fontSize=\'10pt\'" ONCLICK="window.parent.proc_copiar_excel(); window.parent.oPopup.hide()" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/Buttons/btn_Excel.png" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Excel</label></TD></TR>';

html+='<TR><TD STYLE="border:1pt solid navy" ID="btnPrint" ONMOUSEOVER="document.all.btnPrint.style.background=\'#CFD6E8\';document.all.btnPrint.style.border=\'1pt solid #737B92\';document.all.btnPrint.style.color=\'black\';" ONMOUSEOUT="document.all.btnPrint.style.background=\'navy\';document.all.btnPrint.style.border=\'1pt solid navy\';document.all.btnPrint.style.color=\'white\';document.all.btnPrint.style.fontSize=\'10pt\'" ONCLICK="window.parent.print(); window.parent.oPopup.hide()"  UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/Buttons/btn_Print.png" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Imprimir</label></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy"><IMG SRC="pixel.gif" WIDTH="130" HEIGHT="1"></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy" ID="i8" ONMOUSEOVER="document.all.i8.style.background=\'#CFD6E8\';document.all.i8.style.border=\'1pt solid #737B92\'; document.all.i8.style.color=\'black\';" ONMOUSEOUT="document.all.i8.style.background=\'navy\';document.all.i8.style.border=\'1pt solid navy\'; document.all.i8.style.color=\'white\';document.all.i8.style.fontSize=\'10pt\'" ONCLICK="if(window.parent.document.body.style.zoom!=0) window.parent.document.body.style.zoom*=1.6; else window.parent.document.body.style.zoom=1.6;" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/menuzoom.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Zoom In </label></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy" ID="i9" ONMOUSEOVER="document.all.i9.style.background=\'#CFD6E8\';document.all.i9.style.border=\'1pt solid #737B92\'; document.all.i9.style.color=\'black\';" ONMOUSEOUT="document.all.i9.style.background=\'navy\';document.all.i9.style.border=\'1pt solid navy\'; document.all.i9.style.color=\'white\';document.all.i9.style.fontSize=\'10pt\'" ONCLICK="if(window.parent.document.body.style.zoom!=0) window.parent.document.body.style.zoom*=0.625; else window.parent.document.body.style.zoom=0.625;" UNSELECTABLE="on">&nbsp;<IMG src="../../../../Images/menuzoom.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Zoom Out</label></TD></TR>';


//html+='<TR><TD STYLE="border:1pt solid navy" ID="i20" ONMOUSEOVER="document.all.i20.style.background=\'#CFD6E8\';document.all.i20.style.border=\'1pt solid #737B92\'; document.all.i20.style.color=\'black\'; document.all.i20.style.fontSize=\'12pt\'" ONMOUSEOUT="document.all.i20.style.background=\'navy\';document.all.i20.style.border=\'1pt solid navy\'; document.all.i20.style.color=\'white\';document.all.i20.style.fontSize=\'10pt\'" ONCLICK="window.parent.oPopup.hide()" UNSELECTABLE="on">&nbsp;<IMG SRC="menuzoom.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;<label style="cursor:\'default\'">Zoom Out</label></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy"><IMG SRC="pixel.gif" WIDTH="130" HEIGHT="1"></TD></TR>';

//html+='<TR><TD STYLE="border:1pt solid navy" ID="i12" ONMOUSEOVER="document.all.i9.style.background=\'#CFD6E8\';document.all.i9.style.border=\'1pt solid #737B92\';" ONMOUSEOUT="document.all.i9.style.background=\'navy\';document.all.i9.style.border=\'1pt solid navy\';" ONCLICK="if(window.parent.document.body.style.zoom!=0) window.parent.document.body.style.zoom*=0.625; else window.parent.document.body.style.zoom=0.625;">&nbsp;<IMG SRC="menuzoom.gif" WIDTH="20" HEIGHT="20" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="absmiddle">&nbsp;Inicio</TD></TR>';
...
if(isie)
	...
	oPopup.show(x, y, 140, 120, document.body);
	}
}

function click(e) 
{
if(isie && document.all && (event.button==2 || event.button==3))
	{
...
if (objetos.length)
	...
	for (i=objetos.length-1; i>=0; --i)
		...
		if (objetos[i].tagName=='B' || objetos[i].tagName=='TD' || objetos[i].tagName=='TH')
			{
			if (objetos[i].className!="texto" && (objetos[i].innerText.search(/^\$?(\d{1,3}(?:,\d{3})*(?:\.\d+)?)$/g))!=-1)
				{
...
try {
	oPage.location.href=oPage.location.href;
	if (isModalWindow || 1==1)
		{
		oPage.document.location.href=oPage.document.location.href;
		}
	else
		{
		with (oPage)
	...
	catch(e2) {alert('No se pudo actualizar la página: \n'+e.description)}}
}
...
function modalOperations(oSource)
{
sKeyCode=event.keyCode;
if (sKeyCode==45 && oSource.allowInsert) //Ins
	...
	if (oSource.dataValue!='NULL') 
		openEditPage('../Templates/request.asp?catalogName='+oSource.catalogName+'&mode=update&Filters='+oSource.dataValue+'='+getVal(oSource)); 
	event.keyCode=0;
	event.returnValue=false;
	}
...
/*if ( eval("paginaActual().search(/"+sLink.replace('/', '\\').replace('\.', '\\.')+"/g)")!=-1 )
	{
	openLinksLink, false, true); 
	}
else
	{*/
	registro=openLinksLink, true, false); 
	if (registro!=undefined && !esVacio(registro))
		{
		eval(registro);
		}
//	}
...
{
return openLinksLink, true, false); 
}
...
function openLinksLink, bPopUp, bReplace)
{
...
//	return var_str.replace(/^(\w*\()(.*)(\))$/gi, '$2')
	...
	myObject.pageURL = /*(sLink.search(/^http:/gi)!=-1?'':rootFolder)+*/sLink+"&sid="+Math.random();
	var resultados=window.showModalDialog("../App_Layout/ModalInterface.asp", myObject, "help:no; status=yes; resizable:yes; dialogHeight:"+String(document.body.offsetHeight+30)+"px; dialogWidth:"+String(document.body.offsetWidth+6)+"px; dialogLeft:114px; dialogTop:160px; ");
...
switch (sControlType) {
	...
	loadXMLFile(sXMLFile, (oContext.templateName, getContextSchema(oContext)), oContext)
...
{
var sFilters=window.showModalDialog('../App_Layout/filtersTest.asp?sid='+Math.random()+'&requestTable='+sTableName/*+'&sid='+Math.random()*/, null, "help:no; status=yes; resizable:yes; dialogHeight:"+String(document.body.offsetHeight+30)+"px; dialogWidth:"+String(document.body.offsetWidth/2)+"px; dialogLeft:100px; dialogTop:100px; ");
return sFilters;
/*
...
/*function GetFilters(sTableName)
{
var sFilters=window.showModalDialog(rootFolder+'DynamicData/Filters.asp?requestTable='+sTableName/*+'&sid='+Math.random()**, null, "help:no; status=yes; resizable:yes; dialogHeight:"+String(document.body.offsetHeight+30)+"px; dialogWidth:"+String(document.body.offsetWidth/2)+"px; dialogLeft:100px; dialogTop:100px; ");
if (sFilters!=undefined && !esVacio(sFilters))
	{
	return encodeURIComponent(sFilters);
	}
else
	{
	return '';
...
return (!( oSrcElement==oElement || oSrcElement!=oElement && ((oElement.name || oElement.id || true)!=(oSrcElement.name || oSrcElement.id || true)) )) 
}
...
function Login()
{
	...
	this.go = function()
		{
		var myObject = new Object();
		var oResult=window.showModalDialog(this.URL+"?sid="+Math.random(), myObject, "help:no; status=yes; resizable:yes; unadorned:yes; center:yes; dialogHeight:"+this.height+"px; dialogWidth:"+this.weight+"px;"); // dialogLeft:114px; dialogTop:160px; 
		this.result=oResult
	...
	this.logout = function()
		...
		self.parent.close() //window.close()
...
function createUpdateXML(jTable)
{
var sUpdateXML = ""
if (jTable==undefined) jTable=$(".dataTable")//.not(".dataTable .dataTable")
//jTable.find(".dataTable").css("background-color", "red")

//alert('Tables: '+$("[db_identity_value]", jTable).length)

var oTable=jTable.get(0);
jTable.parent().find("[db_identity_value]").filter(function(){return $(this).closest(".dataTable").get(0)==oTable}).each(function(index,oIdentity){
	...
	oInputCollection.each(function(){
		try { 
			if (eval(this.isSubmitable))
				{
				//alert(this.name+': '+this.isSubmitable)
				var_column_value=getDBColumnValue(this);
				...
				xmlInputCollection.push(element('dataField', var_column_value/*getVal(this)*/, {
				"name":this.dataField
				})) 
			}
		} catch(e) {}
	});
	var oChildren=$(".dataTable", jTable)
	if (oChildren.length>0) xmlInputCollection.push(createUpdateXML(oChildren))
	sUpdateXML+=element('dataRow', xmlInputCollection.join(' '), {
		"identityValue":oIdentity.db_identity_value,
		"sourceObjectId":oIdentity.id
		})
	})
	if (oTable.db_foreign_key)
		{
		sUpdateXML=element('dataTable', sUpdateXML, {
			"name":oTable.db_table_name,
			"primaryKey":oTable.db_primary_key,
			"identityKey":oTable.db_identity_key,
			"foreignKey":oTable.db_foreign_key
			})
		}
	else
		{
...
{
xmlData=createUpdateXML()
uploadXMLFile("../Scripts/ajax_updateDB.asp", xmlData, oTable)
}
/*
var xmldata = "<root/>"
...
xmlhttp.setRequestHeader("Content-Disposition", "form-data; name=\"version\"");
xmlhttp.send("3");

//open again for the second package
xmlhttp.open(method, url, false);
xmlhttp.setRequestHeader("Content-Disposition", "form-data; name=\"datafile\"");
xmlhttp.setRequestHeader("Content-Type", "application/xml");
xmlhttp.send(xmldata);You can use xmlhttp.responseXML to get the xml data returned by the device.*/
