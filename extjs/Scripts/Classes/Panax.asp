...
Class Panax
	...
	Private Sub DataBind()
		DIM rsRecordSet
		set oXMLFile = Server.CreateObject("MSXML2.DOMDocument")
		...
		Dim fso:	Set fso = Server.CreateObject("Scripting.FileSystemObject")
		DIM sApplicacionOutput: sApplicacionOutput=sOutput
		IF UCASE(sOutput)="JSON" THEN 
			sApplicacionOutput=fso.GetFolder(server.MapPath("..")).name
		END IF
		...
		'ON ERROR RESUME NEXT
			...
			CASE "HTML":
				...
				IF bGetStructure="" THEN bGetStructure = 1 END IF
			CASE "JSON":
				IF bGetData="" THEN bGetData = 1 END IF
				IF bGetStructure="" THEN bGetStructure=0 END IF
			CASE ELSE 
...
catalog: {
	...
	Public Sub HTMLFormat() 
		...
		IF (oXMLFile.documentElement.getAttribute("controlType")="fileTemplate") THEN 
			...
			END IF
			Document.FileName=server.MapPath("..\..\..\..\Docs\"&FileTemplate)
