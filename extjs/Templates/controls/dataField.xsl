...
<xsl:template mode="emptyText" priority="-2" match="*[@emptyText]"><xsl:value-of select="@emptyText" disable-output-escaping="yes"/></xsl:template>
...
<xsl:template mode="labelWidth" match="*" priority="-2">130</xsl:template>
...
<xsl:template match="*[@dataType='foreignTable'][*/@controlType='formView']" mode="dataField">
/*formcontent*/<xsl:apply-templates select="." mode="Form.Content"/>
</xsl:template>
