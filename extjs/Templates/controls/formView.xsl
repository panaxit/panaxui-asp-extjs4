...
<xsl:template match="*[@dataType='table']" mode="formView.table.Template">
	Ext.define('Px.<xsl:value-of select="@dbId"/>.<xsl:value-of select="@xml:lang"/>.<xsl:value-of select="@Table_Schema"/>.<xsl:value-of select="@Table_Name"/>.<xsl:value-of select="translate(@mode, $uppercase, $smallcase)"/>.<xsl:value-of select="@controlType"/>', { 
...
<xsl:template mode="headerText" priority="-1" match="px:fieldSet[@headerText]"><xsl:value-of select="@headerText"/></xsl:template>
...
<xsl:template mode="headerText" priority="-1" match="px:portlet[@headerText]"><xsl:value-of select="@headerText"/></xsl:template>
...
<xsl:template mode="headerText" priority="-1" match="px:tabPanel[@headerText]"><xsl:value-of select="@headerText"/></xsl:template>
...
<xsl:template mode="formView.layout.regions.center" match="px:layout">
	{
		xtype: 'panel'
		,frame: true
		<xsl:if test="not(../..)">,autoScroll: true</xsl:if>
		,defaults: {
			anchor: 'none'
		}
...
<xsl:template mode="formView" match="*[@dataType='foreignTable']/*[@controlType='formView']/px:layout">
	<xsl:apply-templates mode="formView" select="*"/>
</xsl:template>
...
<xsl:template mode="formView" match="px:fieldContainer">
	...
	<xsl:if test="position()&gt;1">,</xsl:if>{
			...
			, itemId: "<xsl:apply-templates mode="container_id" select="."/>"
			, fieldLabel: '<xsl:choose><xsl:when test="@headerText"><xsl:value-of select="@headerText"/></xsl:when><xsl:otherwise><xsl:if test="not(ancestor::*[@mode='fieldselector'])"><xsl:apply-templates select="." mode="headerText"/></xsl:if></xsl:otherwise></xsl:choose>'
...
<xsl:template match="px:field|px:fields/*" mode="fieldContainer">
	...
	<xsl:if test="position()&gt;1">,</xsl:if>/*<xsl:value-of select="key('fields', px:field/@fieldId)[@dataType='bit']"/>*/{
		<xsl:choose>
			<xsl:when test="$field/@relationshipType='hasOne' and $field/@dataType='foreignTable'">
				xtype: "fieldset", title: '<xsl:if test="not(ancestor::*[@mode='fieldselector'])"><xsl:apply-templates select="$field" mode="headerText"/></xsl:if>'
				, layout: 'vbox'
				, resizable: true
				, defaults: { labelAlign:'left', hideLabel: false, anchor: 'none', flex: 0, <xsl:apply-templates select="$field" mode="container.items.defaultConfig"/>}
			</xsl:when>
			<xsl:otherwise>
				xtype: "fieldcontainer", fieldLabel: '<xsl:if test="not(ancestor::*[@mode='fieldselector'])"><xsl:apply-templates select="$field" mode="headerText"/></xsl:if>'
				, layout: 'hbox'
